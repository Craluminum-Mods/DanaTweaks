...
public static class CollectibleObject_OnHeldUseStart_Patch
{
    public static bool Prefix(ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EnumHandInteract useType)
    {
        ...
        ItemSlot bestSlot =
            ...
            .FirstOrDefault(x => IsFaster(thisObject: x?.Itemstack?.Collectible, otherObject: slot?.Itemstack?.Collectible, targetMaterial), defaultValue: slot);

        if (bestSlot != null && (slot?.Itemstack?.Collectible.Equals(slot?.Itemstack, bestSlot.Itemstack, GlobalConstants.IgnoredStackAttributes)) == false)
        {
